...
<body class="v2-report operator-dashboard"><div class="shell">
  ...
  <header class="hero area-1"><div class="hero-main"><div class="status-line"><span class="area-kicker">AREA 1</span><span class="status-badge">今の結論</span><span class="safety">REPORT ONLY / HUMAN DECISION</span></div><div class="decision-grid"><div class="decision-word{decision_word_class}">{html.escape(decision_word)}</div><div class="decision-copy"><h1>{html.escape(conclusion)}</h1><p>{html.escape(' / '.join(reasons[:2]) or '価格帯と15分足の反応を確認します。')}</p></div></div><p class="boundary-note">この報告書は表示専用です。売買許可・確率・勝率を示さず、最終判断と注文は人間が行います。</p></div><div class="hero-side"><div class="current-label">現在値 / 更新時刻</div><div class="current-price">{_format_operator_price(result.get('current_price'))}</div><div class="current-timestamp">{html.escape(timestamp)}</div><div class="metric-stack">{metrics}</div><p class="metric-note">機械評価インデックスであり、確率・勝率・実行許可ではありません。</p></div></header>
  <section class="area-2 panel"><div class="section-heading"><div><span class="area-kicker">AREA 2</span><h2>時間軸別の方向</h2></div><strong class="alignment-summary">{html.escape(view["alignment_summary"])}</strong></div><div class="timeframe-grid">{tf_cards}</div>{_operator_report_score_summary(result, view)}</section>
