...
def _validate_integrity(scenarios: list[dict[str, str]], events: list[dict[str, str]], classes: list[dict[str, str]]) -> str | None:
    ...
    for row in events:
        if not row.get("candidate_id") or (row.get("grouping_status") == "ambiguous" and row.get("scenario_id")) or (row.get("grouping_status") != "ambiguous" and row.get("scenario_id") not in known):
            return "input_schema_mismatch"
        event_utc = _dt(row.get("event_timestamp_utc")); event_jst = _dt(row.get("event_timestamp_jst"))
        if not event_utc or not event_jst or abs((event_utc - event_jst).total_seconds()) > 1:
            return "invalid_timestamp"
    ids = [row.get("classification_id", "") for row in classes]
    if any(not x for x in ids):
        return "input_schema_mismatch"
    by_class_id: dict[str, dict[str, str]] = {}
    for row in classes:
        identity = row["classification_id"]
        normalized = {key: value for key, value in row.items() if key != "classification_id"}
        if identity in by_class_id:
            if by_class_id[identity] == normalized:
    ...
    if len(set(refs)) != len(refs):
        ...
        for row in classes:
            event_ref = row.get("scenario_event_id", "")
            normalized = {key: value for key, value in row.items() if key not in {"scenario_event_id", "classification_id"}}
            if event_ref in grouped and grouped[event_ref] != normalized:
                return "classification_assignment_conflict"
    ...
    for row in classes:
        ...
        if row.get("classification_status") == "classified" and not row.get("operator_class"):
            return "input_schema_mismatch"
        if event.get("grouping_status") == "ambiguous" and row.get("classification_status") != "ambiguous_grouping":
            return "input_schema_mismatch"
        if event.get("grouping_status") != "ambiguous" and row.get("classification_status") == "ambiguous_grouping":
            return "input_schema_mismatch"
    threshold_fields = ("short_direction_min", "short_execution_min", "short_wait_max", "short_tp1_rr_min", "short_tp2_rr_min", "long_direction_min", "long_execution_min", "long_wait_max", "long_tp1_rr_min", "long_tp2_rr_min")
    for row in classes:
        if any(_dec(row.get(field)) is None for field in threshold_fields):
            return "invalid_threshold_snapshot"
    return None
...
def _effective_decisions(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], str | None]:
    ...
    if len(superseded) != len(set(targets)) or len(targets) != len(set(targets)):
        return [], "invalid_correction_graph"
    effective = [row for row in rows if row["decision_event_id"] not in superseded]
    effective.sort(key=lambda row: (_dt(row.get("human_checked_at_utc") or row.get("human_checked_at_jst")) or datetime.max.replace(tzinfo=timezone.utc), row.get("decision_event_id", "")))
...
def build_manual_operator_historical_replay(*, scenarios: Path, scenario_events: Path, classifications: Path, output_csv: Path, output_json: Path, output_md: Path, report_date: str, decision_events: Path | None = None, trade_episodes: Path | None = None, episode_links: Path | None = None, dry_run: bool = False, replace_output: bool = False) -> dict[str, Any]:
    ...
    if episode_rows:
        ...
        if any(not value for value in episode_ids) or len(episode_ids) != len(set(episode_ids)):
            return {"ok": False, "exit_code": 2, "errors": ["invalid_episode_identity"], "report_written": False}
        for row in episode_rows:
            opened = _dt(row.get("opened_at_utc") or row.get("opened_at_jst")); closed = _dt(row.get("closed_at_utc") or row.get("closed_at_jst"))
            if not opened or (row.get("status") == "closed" and not closed) or (opened and closed and closed < opened):
                return {"ok": False, "exit_code": 2, "errors": ["invalid_episode_timestamp"], "report_written": False}
            for field in ("realized_pnl", "fee_total"):
                if row.get(field) and _dec(row[field]) is None:
    ...
    for policy in POLICIES:
        ...
        for cls in selected:
            ...
            eligible_links = [link for link in link_rows if replay["row_role"] == "entry_candidate" and link.get("signal_id") == event.get("source_signal_id") and link.get("link_status") == "linked" and link.get("link_confidence") in {"high", "medium"} and link.get("side_compatibility") == "match" and link.get("symbol_compatibility") in {"match", "unknown"}]
            if eligible_links:
                ...
                if len(eligible_links) != 1:
                    ...
                    replay["actual_join_status"] = "matched"; replay["actual_episode_id"] = episode.get("episode_id", ""); replay["actual_link_confidence"] = eligible_links[0].get("link_confidence", "")
                    if episode.get("status") == "closed":
                        replay["actual_gross_realized_pnl"] = episode.get("realized_pnl", ""); replay["actual_fee_total"] = episode.get("fee_total", "")
                    else:
                        replay["actual_join_status"] = "open_episode_observation"
                    gross = _dec(episode.get("realized_pnl")); fee = _dec(episode.get("fee_total"))
                    if episode.get("status") == "closed" and gross is not None and fee is not None:
                        replay["actual_net_pnl_after_fee"] = str(gross - abs(fee))
    ...
    for policy in POLICIES:
        ...
        for row in selected_policy:
            ...
            if len(signal_scenarios.get(signal_id, set())) > 1:
                row["actual_join_status"] = "ambiguous"; row["actual_episode_id"] = ""; row["actual_link_confidence"] = ""; row["actual_gross_realized_pnl"] = ""; row["actual_fee_total"] = ""; row["actual_net_pnl_after_fee"] = ""
        seen_episodes: set[str] = set()
        for row in sorted(selected_policy, key=lambda item: (item.get("selected_at_utc", ""), item.get("scenario_id", ""), item.get("replay_row_id", ""))):
            episode_id = row.get("actual_episode_id", "")
            if not episode_id:
                continue
            if episode_id in seen_episodes:
                row["actual_join_status"] = "duplicate_episode_excluded"; row["actual_episode_id"] = ""; row["actual_link_confidence"] = ""; row["actual_gross_realized_pnl"] = ""; row["actual_fee_total"] = ""; row["actual_net_pnl_after_fee"] = ""
            else:
    ...
    strict_ids = {row["scenario_id"] for row in rows if row["policy_name"] == "CURRENT_STRICT"}
    over_suppression = sum(1 for row in rows if row["policy_name"] == "A_PLUS_B" and row["selected_operator_class"] == "B_CHECK_15M" and row["normalized_outcome_status"] == "resolved_positive" and row["scenario_id"] not in strict_ids)
    ...
    for path, marker in ((output_csv, "schema_version"), (output_json, '"schema_version"'), (output_md, "# Manual Operator Historical Replay")):
        if path.exists() and not replace_output:
            try:
                ...
                if path == output_json and json.loads(text).get("schema_version") != REPORT_SCHEMA_VERSION: return {"ok": False, "exit_code": 4, "errors": ["existing_output_schema_mismatch"], "report_written": False}
                if path == output_md and not text.startswith("# Manual Operator Historical Replay\n"): return {"ok": False, "exit_code": 4, "errors": ["existing_output_schema_mismatch"], "report_written": False}
            except (OSError, UnicodeError, ValueError, csv.Error): return {"ok": False, "exit_code": 4, "errors": ["existing_output_schema_mismatch"], "report_written": False}
