...
def _read(path: Path, required: tuple[str, ...]) -> tuple[list[dict[str, str]], str | None]:
    if not path.exists():
...
def _signal_class(row: dict[str, Any]) -> str:
    ...
    if any(token in text for token in ("defensive", "wait", "watch", "見送り", "監視")):
        return "defensive"
    return "entry_like"
...
def build_manual_trade_signal_link_rows(*, episodes: list[dict[str, Any]], signal_rows: list[dict[str, Any]], signal_outcome_rows: list[dict[str, Any]], max_lookback_minutes: int = 240) -> list[dict[str, str]]:
    ...
    for row in signal_outcome_rows + signal_rows:
        ...
        if signal_id:
            merged.setdefault(signal_id, {}).update(row)
    ...
    for episode in episodes:
        ...
        output.append({
            ...
            "link_method_version": LINK_METHOD_VERSION, "created_at_utc": "1970-01-01T00:00:00Z",
        })
...
def link_manual_trade_episodes_to_signals(*, episodes: Path, signals: Path, signal_outcomes: Path, output_csv: Path | None = None, max_lookback_minutes: int = 240, dry_run: bool = False, replace_output: bool = False) -> dict[str, Any]:
    ...
    episode_rows, episode_error = _read(episodes, ("schema_version", "episode_id"))
    signal_rows, signal_error = _read(signals, ("signal_id",))
    outcome_rows, outcome_error = _read(signal_outcomes, ("signal_id",))
    errors = [error for error in (episode_error, signal_error, outcome_error) if error]
    summary = {"ok": False, "schema_version": SCHEMA_VERSION, "link_method_version": LINK_METHOD_VERSION, "dry_run": dry_run, "episode_count": len(episode_rows), "linked_count": 0, "output_csv": output.name, "errors": errors, "safety_boundary": "report-only / not FORMAL_GO / no automatic order / human decides manually"}
    if errors:
        summary["exit_code"] = 2
        return summary
