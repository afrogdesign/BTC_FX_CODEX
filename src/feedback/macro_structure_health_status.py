...
def _artifact(root: Path, latest_name: str, identity: str, files: tuple[str, ...], schema: str, method: str, symbol: str, prefix: str) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], Path, dict[str, str]]:
    ...
    fingerprints = {name: _sha256(artifact / name) for name in files}
...
def _validate_runtime(path: Path, symbol: str | None = None) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    _require(isinstance(steps, list) and [item.get("name") for item in steps] == ["snapshot", "history", "operator"], "runtime_step_order")
    for item in steps:
    ...
    _require(isinstance(fingerprints, dict) and all(isinstance(fingerprints.get(key), str) and fingerprints[key] for key in ("15m", "1h", "4h")), "runtime_public_inputs_invalid")
    ...
    return status, {f"runtime:{key}": value for key, value in fingerprints.items()}
...
def _markdown(payload: dict[str, Any]) -> str:
    return "\n".join([
        ...
        f"- warning: stale={payload.get('stale_status', 'unavailable')}, continuity={payload.get('continuity_status', 'unavailable')}, snapshot={payload.get('snapshot_result_status', 'unavailable')}",
...
def check_macro_structure_health(*, runtime_status: Path = Path("logs/runtime/macro_structure_service_last_result.json"), snapshot_root: Path = Path("local/reports/macro_structure"), history_root: Path = Path("local/reports/macro_structure/history"), operator_root: Path = Path("local/reports/macro_structure/operator"), plist: Path = Path("deploy/com.afrog.btc-macro-structure.plist"), output_root: Path = Path("local/reports/macro_structure/health"), evaluation_time_utc: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        if status.get("status") == "success":
            ...
            source_fingerprints.update({f"artifact:{key}": value for key, value in artifact_fp.items()})
        else:
        ...
        payload: dict[str, Any] = {
            ...
            "snapshot_result_status": status.get("snapshot_result_status"), "history_result_status": status.get("history_result_status"), "operator_result_status": status.get("operator_result_status"),
            ...
            "stale_timeframes": (snapshot.get("stale_timeframes") if snapshot else []), "latest_as_of_utc": snapshot.get("as_of_utc") if snapshot else None,
            ...
            "runtime_status_path": _relative(runtime_status), "operator_html_path": f"local/reports/macro_structure/operator/{operator.get('artifact_dir')}/macro_structure_operator.html" if operator else None,
        ...
        payload = {
            ...
            "symbol": symbol, "error_code": exc.code, "reason_codes": [exc.code], "source_fingerprints": source_fingerprints,
    ...
    return {"ok": exit_code == 0, "exit_code": exit_code, "report_written": True, "health_state": payload["health_state"], "severity": payload["severity"], "health_artifact_id": health_id, "artifact_dir": _relative(artifact), "evaluation_utc": payload["evaluation_utc"], "evaluation_jst": payload["evaluation_jst"], "symbol": symbol, "runtime_status": payload.get("runtime_status"), "snapshot_run_id": payload.get("snapshot_run_id"), "snapshot_id": payload.get("snapshot_id"), "history_id": payload.get("history_id"), "operator_artifact_id": payload.get("operator_artifact_id"), "snapshot_result_status": payload.get("snapshot_result_status"), "stale_status": payload.get("stale_status"), "continuity_status": payload.get("continuity_status"), "data_quality_status": payload.get("data_quality_status"), "report_only": True, "private_actual_trade_input": False, "automatic_order_allowed": False}
