...
def evaluate_readiness(manifest: Mapping[str, Any], trial_report: Mapping[str, Any], cumulative_manifest: Mapping[str, Any], modern_report: Mapping[str, Any], claim_scope: Mapping[str, Any] | None = None, frozen_validation: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    op_state = manifest.get("operational_health", {}).get("state", "blocked_data")
    ...
    elif validation_status == "passed" and proposal_state == "eligible_for_proposal" and validation.get("generation_compatible") is True and validation.get("cohort_compatible") is True and validation.get("time_split") is True:
        adoption_state = "human_approval_required"
    else:
        adoption_state = "collecting"
    dimensions = {
        "operational_health": _dimension(op_state, {"daily_state": op_state}, [] if op_state == "healthy" else ["daily operational health"], scope),
        ...
        "proposal_eligibility": _dimension(proposal_state, {"threshold_status": thresholds, "minimum_requirements": minimums}, proposal_missing, scope),
