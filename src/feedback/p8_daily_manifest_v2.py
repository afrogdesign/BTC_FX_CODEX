...
def build_manifest(cycle: Mapping[str, Any], trial: Mapping[str, Any], facts: list[Mapping[str, Any]], queue: list[Mapping[str, Any]], cumulative: Mapping[str, Any], modern: Mapping[str, Any], runtime: str, source: str, cutoff: str, previous: Mapping[str, Any] | None = None, claim_scope: Mapping[str, Any] | None = None, frozen_validation: Mapping[str, Any] | None = None, input_meta: Mapping[str, Mapping[str, Any]] | None = None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    modern_actual = modern.get("actual_attribution", {})
    modern_pointer = {"logical_name": "modern_attribution_report.json", "fingerprint": _s(modern.get("_input_fingerprint")), "baseline_episodes": int(modern.get("baseline_episodes", 0) or 0), "baseline_links": int(modern.get("baseline_links", 0) or 0), "accepted_high_medium_actual": int(modern_actual.get("accepted_high_medium_actual_associations", modern_actual.get("accepted_high_medium", 0)) or 0), "accepted_high_medium_actual_associations": int(modern_actual.get("accepted_high_medium_actual_associations", modern_actual.get("accepted_high_medium", 0)) or 0), "notified_accepted_actual_associations": int(modern_actual.get("notified_accepted_actual_associations", 0) or 0), "unknown_notification_status_accepted_actual_associations": int(modern_actual.get("unknown_notification_status_accepted_actual_associations", 0) or 0), "human_confirmed_usefulness_count": int(modern_actual.get("human_confirmed_usefulness_count", 0) or 0), "metadata_complete_notified_actual_count": int(modern_actual.get("metadata_complete_notified_actual_count", 0) or 0), "operator_direction_match_rate": modern.get("operator_direction", {}).get("operator_direction_match_rate"), "automatic_causal_claims": int(modern.get("causality_statement", {}).get("automatic_causal_claims", 0) or 0), "canonical_link_replacement": modern.get("canonical_link_replacement", False)}
    ...
    if previous is not None and comparable:
        previous_state = _s(previous.get("p9_readiness_v2", {}).get("state")); current_state = _s(readiness.get("state"))
        if previous_state and current_state and previous_state != current_state:
            meaningful.append({"type": "p9_readiness_state_transition", "from": previous_state, "to": current_state})
