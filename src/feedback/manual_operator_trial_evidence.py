...
def _issue_flags(row: dict[str, str], status: str, opposite_available: bool) -> list[str]:
    ...
    if row.get("operator_class") == "STOP_OR_EXIT":
...
def build_manual_operator_trial_evidence(
    *, scenarios: Path, scenario_events: Path, classifications: Path, output_csv: Path,
    ...
    with tempfile.TemporaryDirectory(prefix="p8-replay-") as temp_dir:
        ...
        event_signal_counts = Counter(row.get("source_signal_id", "") for row in event_rows)
        ...
        for event_id, replay_row in sorted(selected.items(), key=lambda item: (item[1].get("selected_at_utc", ""), item[0])):
            ...
            opposite_available = (
                len(opposite_by_signal.get(signal_id, set()) - {side, ""}) > 0
                or str(event_by_id.get(event_id, {}).get("opposite_side_candidate_available", "")).strip().lower() in {"1", "true", "yes"}
            )
            comparison = _status(replay_row)
            ...
            row.update(
                ...
                direction_result=("correct" if comparison == "aligned" else ("wrong" if comparison == "wrong_side" else "")), comparison_status=comparison,
        ...
        issue_summary = {
            "P8-ISSUE-001_GLOBAL_STOP_MASKS_SIDE_OPPORTUNITY": {"occurrence_count": issue_counts.get("P8-ISSUE-001_GLOBAL_STOP_MASKS_SIDE_OPPORTUNITY", 0), "status": "open hypothesis", "tuning_eligibility": "not_eligible"},
            "P8-ISSUE-002_MAIN_VS_BIG_CHANCE_HIERARCHY": {"occurrence_count": 0, "status": "confirmed usability issue", "tuning_eligibility": "not_eligible"},
            "P8-ISSUE-003_RAW_CLASSIFIER_PAYLOAD_EXPOSED": {"occurrence_count": 0, "status": "confirmed usability issue", "tuning_eligibility": "not_eligible"},
            "P8-ISSUE-004_STOP_CARD_SIDE_IDENTITY": {"occurrence_count": 0, "status": "confirmed usability issue", "tuning_eligibility": "not_eligible"},
        }
        report = {"schema_version": SCHEMA_VERSION, "report_date": report_date, "report_written": False, "safety_boundary": SAFETY, "method_version": METHOD_VERSION, "input_status": {"scenarios": "provided", "scenario_events": "provided", "classifications": "provided", "actual": "provided" if trade_episodes else "missing"}, "cutoff": report_date, "counts": {"trial_fact_rows": len(rows), "resolved_rows": resolved, "unresolved_rows": unresolved, "no_ohlcv_rows": no_ohlcv, "ambiguous_rows": sum("ambiguous" in r.get("outcome_status", "") for r in rows), "scenario_count": len({r.get("scenario_id") for r in rows if r.get("scenario_id")}), "review_queue_size": len(queue)}, "class_distribution": dict(sorted(class_counts.items())), "comparison": dict(sorted(comparisons.items())), "zone_result_counts": dict(sorted(Counter(r.get("zone_result", "") for r in rows if r.get("zone_result")).items())), "breakdowns": {"side": dict(sorted(side_counts.items())), "regime": dict(sorted(regime_counts.items())), "setup_family": dict(sorted(setup_counts.items()))}, "actual_evidence": {"status": "provided" if trade_episodes else "missing", "eligible_rows": len(actual_rows), "high_confidence_rows": sum(r.get("actual_link_confidence") == "high" for r in actual_rows), "medium_confidence_rows": sum(r.get("actual_link_confidence") == "medium" for r in actual_rows)}, "link_confidence_coverage": dict(sorted(Counter(r.get("actual_link_confidence", "") for r in rows if r.get("actual_link_confidence")).items())), "issue_flags": dict(sorted(issue_counts.items())), "issue_summary": issue_summary, "review_queue_size": len(queue), "p9_readiness": readiness, "global_stop_opportunity": {"global_stop_present": sum(r.get("operator_class") == "STOP_OR_EXIT" for r in rows) > 0, "stop_rows": sum(r.get("operator_class") == "STOP_OR_EXIT" for r in rows), "opposite_side_issue_rows": issue_counts.get("P8-ISSUE-001_GLOBAL_STOP_MASKS_SIDE_OPPORTUNITY", 0)}, "no_automatic_tuning": True}
