...
def replay_macro_structure_volatility(*, signals: Path, ohlcv_15m: Path, ohlcv_1h: Path, ohlcv_4h: Path, output_events_csv: Path, output_levels_csv: Path, output_misses_csv: Path, output_json: Path, output_md: Path, cutoff_utc: str | None = None, left_window: int = 2, right_window: int = 2, replace_output: bool = False, performance_start_utc: str | None = None, performance_end_utc: str | None = None) -> dict[str, Any]:
    ...
    for signal in signal_rows:
        ...
        reliability_rows = {}
        for level in levels:
            state = _level_state(level, candles1, at); reliability = _reliability(level, state, at); reliability_rows[level["level_id"]] = reliability
            if signal in performance_rows:
                level_rows[level["level_id"]] = reliability
            level["reliability_band"] = reliability["reliability_band"]; level["center"] = reliability["center"]
    ...
    performance_episodes = [row for row in policy_episodes if not (row.get("event") or {}).get("_context_only")]
    def split_metrics(key: str) -> dict[str, Any]:
        values = sorted({str((row.get("event") or {}).get(key, "insufficient")) for row in performance_episodes})
        result = {}
        for value in values:
            subset = [row for row in policy_episodes if (row.get("event") or {}).get("_context_only") or str((row.get("event") or {}).get(key, "insufficient")) == value]
    ...
    def direction_metrics(direction: str) -> dict[str, Any]:
        subset = [row for row in policy_episodes if (row.get("event") or {}).get("_context_only") or row.get("opportunity_direction") == direction]
    ...
    summary = {"schema_version": SCHEMA_VERSION, "method_version": METHOD_VERSION, "generated_at_utc": max((m["max_timestamp"] for m in (meta15, meta1, meta4)), default=""), "input_fingerprints": {"signals": _sha(signals), "ohlcv_15m": _sha(ohlcv_15m), "ohlcv_1h": _sha(ohlcv_1h), "ohlcv_4h": _sha(ohlcv_4h)}, "coverage": {**coverage, "signals": len(performance_rows), "total_signal_rows": len(signal_rows), "context_signal_rows": len(signal_rows) - len(performance_rows), "performance_signal_rows": len(performance_rows), "performance_start_utc": performance_start.isoformat() if performance_start else "", "performance_end_utc": performance_end.isoformat() if performance_end else ""}, "method_parameters": {"left_window": left_window, "right_window": right_window, "cluster_tolerance": "max(0.30*ATR_confirmation,pivot_price*0.0015)", "material_move": "max(2*ATR_15M,event_price*0.005)", "jump_like": "max(3*ATR_15M,event_price*0.01)", "corridor_minimum_atr": 1.0, "policy_episode_dedup": "per-policy fired-state reset, side/family/structure/volatility/location transition, or 3h separation; future outcomes and implicit resolution excluded"}, "counts": {"signals": len(performance_rows), "events": len(events), "levels": len(level_list), "missed_moves": len(misses), "independent_opportunities": len(opportunities)}, "level_summary": dict(Counter(row["reliability_band"] for row in level_list)), "metrics": policy_metrics, "baselines": policy_metrics, "splits": split_data, "location_metrics": split_data["price_location"], "missed_move_counts": dict(Counter(row["root_cause"] for row in misses)), "false_warning_counts": {policy: metric.get("false_warning_rate") for policy, metric in policy_metrics.items()}, "walk_forward": dates, "recommendation_gate": gate, "recommendation_status": gate["status"], "missed_move_root_causes": dict(Counter(row["root_cause"] for row in misses)), "no_automatic_tuning": True, "safety_boundary": SAFETY}
