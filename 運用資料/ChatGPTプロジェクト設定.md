...
### 対象ブランチ
`運用資料/NEXT_TASK.md` に書かれた最新の作業ブランチ名を書く。
...
ChatGPT プロジェクトを新しく開始するときは、次の文面をそのまま使ってよい。

```text
この repo では推測で開始しないでください。
まず repo 内の `運用資料/ChatGPTプロジェクト設定.md` を読み、
次に `運用資料/NEXT_TASK.md` を開いて `現在のブランチ` を正本として確認してください。
ブランチ名は自動推測せず、必ず `NEXT_TASK.md` の記載を採用してください。

その後、
1. `運用資料/reports/report_hub_latest.md`
2. `NEXT_TASK.md` に書かれた最新 raw report
3. `chatgpt/README.md`
4. `chatgpt/analysis/` の継続メモ
の順に読み、未確定の論点を整理してください。

`chatgpt/specs/active/` が空なら実装指示はまだ確定していない前提で進め、
まず `chatgpt/analysis/` を更新してください。
仕様が固まったら、Codex 用の確定仕様を `chatgpt/specs/active/YYYYMMDD_topic.md` に作成してください。

最初の返答では、必ず次の4点だけを短く報告してください。
- 読んだファイル
- `NEXT_TASK.md` で確認した現在の作業ブランチ
- 継続中の設計テーマ
- 次に更新するファイル
```
...
```text
注意:
- `main` や remote の最新 branch 一覧から判断しない
- `運用資料/NEXT_TASK.md` の branch 記載を最優先する
- `運用資料/ChatGPTプロジェクト設定.md` は普遍ルール、最新指示は `NEXT_TASK.md`
- 実装はしない。まず設計整理と仕様化だけ行う
```
