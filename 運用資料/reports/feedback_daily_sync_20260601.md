...
## 1. まず結論
- 今回の完了データは 33 件です。近似PF は 1.49、全体勝率は 66.7% でした。
- 事後評価では「待つ判断に使えた」が最も多く、24 件でした。
- 平均の役立ち度は 3.64 / 5 でした。
- 根拠整合の入力率は 97.0%、整合した比率は 100.0% でした。
...
## 2. 今回の対象
- 集計期間: 2026-05-25 21:05 〜 2026-05-30 23:05
- 総観測件数: 33
- データ品質の内訳: ok=33
- 近似PF: 1.49
...
- ready阻害理由: confidence_below_min=16件, entry_zone_not_reached=11件, near_entry_zone_waiting_trigger=4件, inside_entry_zone_with_trigger=2件
...
- 根拠整合の入力率: 97.0% / 整合率: 100.0%
- SL評価: SL が狭すぎた=9件, SL は妥当=23件, SL が広すぎた=1件
- TP評価: TP が遠すぎた=7件, TP は妥当=22件, TP が近すぎた=4件
- 4時間足評価: 一部弱い=24件, 妥当=9件
- 1時間足評価: 一部弱い=28件, 妥当=5件
- 15分足評価: 弱い=12件, 妥当=15件, 一部弱い=6件
### 改善アクション
- 分類: 通知文面を調整=2件, 観測継続=9件, 入口条件を調整=21件, 出口設計を調整=1件
- 重要度: 高=9件, 中=21件, 低=3件
- 高優先の代表例:
...
## 5. 改善候補
1. 15分足の執行価格精度が弱い
   理由: tf_15m_eval=poor が 12/33 件 (36.4%)
...
- range: 勝率=66.7%, 平均MFE=7.37, 平均MAE=4.78 (n=21) / データ不足 21/30
...
- normal: 勝率=66.7%, 平均MFE=7.00, 平均MAE=4.72 (n=33)
...
- SWEEP_WAIT: 勝率=87.5%, 平均MFE=8.48, 平均MAE=3.70 (n=8) / データ不足 8/30
- NO_TRADE_CANDIDATE: 勝率=64.3%, 平均MFE=5.83, 平均MAE=5.50 (n=14) / データ不足 14/30

### bias別件数・勝率
- long: 勝率=40.0% (n=5) / データ不足 5/30
- short: 勝率=71.4% (n=28) / データ不足 28/30

### bias別 direction 正誤
- long: correct=1, wrong=3, unclear=1 / wrong_rate=60.0% (n=5)
- short: correct=11, wrong=11, unclear=6 / wrong_rate=39.3% (n=28)

### 成績指標
- 全体勝率: 66.7%
- 平均MFE(signal_based): 7.00
...
- trend_flip_confirmed_down: wrong_rate=60.0% (wrong=6/10)
- failed_breakout_up_reversal: wrong_rate=60.0% (wrong=3/5)
- lower_liquidity_close: wrong_rate=60.0% (wrong=3/5)
- bid_wall_close: wrong_rate=50.0% (wrong=7/14)
- long_flush_exhaustion: wrong_rate=44.4% (wrong=4/9)
- major_support_rejection: wrong_rate=44.4% (wrong=4/9)
- short_into_major_support: wrong_rate=42.3% (wrong=11/26)
- long_into_major_resistance: wrong_rate=40.0% (wrong=8/20)
...
- support_to_resistance_flip: wrong_rate=36.4% (wrong=8/22)
- upper_liquidity_close: wrong_rate=34.8% (wrong=8/23)
...
- 観測候補全体: 9件 / 勝率=66.7% / TP1先行=75.0% / 近似PF=3.42 / 平均MFE=9.57 / 平均MAE=2.80
- setup_watch_learning: 9件 / 勝率=66.7% / TP1先行=75.0% / 近似PF=3.42 / 平均MFE=9.57 / 平均MAE=2.80
...
### market_map
- 記録あり: 33件
- primary_state: early_down=16件, confirmed_down=10件, confirmed_up=5件, near_major_resistance=1件, active_support=1件
- flags: short_into_major_support=26件, support_to_resistance_flip=22件, support_to_resistance_retest_confirmed=21件, long_into_major_resistance=20件, trend_flip_early_down=16件, major_resistance_rejection=12件, trend_flip_confirmed_down=10件, major_support_rejection=9件
- trend_state: early_down=16件, confirmed_down=10件, confirmed_up=5件
- 下方向反転系: 26件 / 勝率=69.2% / wrong_rate=34.6%
- 下方向反転系 平均MFE24h=7.90 / 平均MAE24h=3.75
...
- 主なブロック理由: phase1_inactive=33件, setup_not_ready=33件, no_trade_flags_present=27件, wait_pressure_too_high=16件, execution_shadow_too_low=12件
- paper_orders planned: 0件
- phase1_v1_shadow 記録付き: 33件
- opportunity_gate=pass: 14件
- quality guard blocked: 1件 / 理由=require_execution_for_high_wait=1件
- hard_quality_blocked: 1件 / 理由=require_execution_for_high_wait=1件
- soft_quality_risk: 1件 / 理由=soft_risk:suppress_long_high_wait+suppress_trend_flip_up_strong=1件
- market_map opportunity before/after guard: 24件 -> 5件
- market_map opportunity before/after hard guard: 24件 -> 5件
...
### risk_flags 有効性比較
- trend_flip_confirmed_down: negative_rate=90.0% (n=10)
- bid_wall_close: negative_rate=78.6% (n=14)
- long_into_major_resistance: negative_rate=75.0% (n=20)
- orderbook_bid_heavy: negative_rate=75.0% (n=16)
- short_into_major_support: negative_rate=73.1% (n=26)
- support_to_resistance_flip: negative_rate=72.7% (n=22)
- support_to_resistance_retest_confirmed: negative_rate=71.4% (n=21)
- upper_liquidity_close: negative_rate=65.2% (n=23)
