...
## 1. まず結論
- 今回の完了データは 24 件です。近似PF は 2.18、全体勝率は 75.0% でした。
- 事後評価では「待つ判断に使えた」が最も多く、15 件でした。
- 平均の役立ち度は 3.50 / 5 でした。
- 根拠整合の入力率は 79.2%、整合した比率は 100.0% でした。
...
## 2. 今回の対象
- 集計期間: 2026-05-01 07:05 〜 2026-05-07 00:05
- 総観測件数: 24
- データ品質の内訳: ok=24
- 近似PF: 2.18
...
- ready阻害理由: entry_zone_not_reached=14件, confidence_below_min=6件, near_entry_zone_waiting_trigger=3件, inside_entry_zone_with_trigger=1件
...
### AI事後評価 health
- 状態: backlogあり
- 候補件数: eligible=242 / backlog=55 / AI済み=187 / human_override=0
...
- uptrend: 勝率=83.3%, 平均MFE=7.90, 平均MAE=3.38 (n=12) / データ不足 12/30

### signal_tier別件数・勝率・平均MFE・平均MAE
- normal: 勝率=75.0%, 平均MFE=8.26, 平均MAE=3.78 (n=24) / データ不足 24/30
...
- NO_TRADE_CANDIDATE: 勝率=80.0%, 平均MFE=7.51, 平均MAE=2.89 (n=10) / データ不足 10/30

### bias別件数・勝率
- long: 勝率=78.3% (n=23) / データ不足 23/30
- short: 勝率=0.0% (n=1) / データ不足 1/30

### bias別 direction 正誤
- long: correct=12, wrong=7, unclear=4 / wrong_rate=30.4% (n=23)
- short: correct=0, wrong=1, unclear=0 / wrong_rate=100.0% (n=1)

### 成績指標
- 全体勝率: 75.0%
- 平均MFE(signal_based): 8.26
- 平均MAE(signal_based): 3.78
...
- sweep_incomplete: wrong_rate=35.3% (wrong=6/17)
- short_cover_risk: wrong_rate=33.3% (wrong=3/9)
- lower_liquidity_close: wrong_rate=23.5% (wrong=4/17)
...
- direction_execution_conflict の主な risk_flags: ask_wall_close=3件, lower_liquidity_close=3件, sweep_incomplete=3件
- suppress_reason の内訳: watch_sweep_recheck_wait=4件, no_material_change=3件, confidence_below_long_min=2件
- ENTRY_OK + invalid: 0件
...
- 主な観測ブロック理由: no_trade_candidate=10件, confidence_below_min=6件
...
- 主なブロック理由: phase1_inactive=24件, setup_not_ready=24件, execution_shadow_too_low=11件, wait_pressure_too_high=11件
- paper_orders planned: 0件
- phase1_v1_shadow 記録付き: 24件
- tp_eval=too_close のうち shadow TP1 が現行TP1より遠い候補: 5/5件

### risk_flags 有効性比較
- orderbook_ask_heavy: negative_rate=50.0% (n=10)
- sweep_incomplete: negative_rate=47.1% (n=17)
- lower_liquidity_close: negative_rate=41.2% (n=17)
