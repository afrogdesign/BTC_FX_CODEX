...
## 1. まず結論
- 今回の完了データは 37 件です。近似PF は 0.97、全体勝率は 75.7% でした。
- 事後評価では「待つ判断に使えた」が最も多く、15 件でした。
- 平均の役立ち度は 3.62 / 5 でした。
- 根拠整合の入力率は 40.5%、整合した比率は 100.0% でした。
...
## 2. 今回の対象
- 集計期間: 2026-04-10 03:05 〜 2026-04-15 23:05
- 総観測件数: 37
- データ品質の内訳: ok=37
- 近似PF: 0.97
...
- ready阻害理由: rr_below_min=31件, entry_zone_not_reached=3件, confidence_below_min=1件, inside_entry_zone_with_trigger=1件, near_entry_zone_with_trigger=1件
...
- uptrend: 勝率=77.3%, 平均MFE=4.55, 平均MAE=7.48 (n=22) / データ不足 22/30

### signal_tier別件数・勝率・平均MFE・平均MAE
- normal: 勝率=75.7%, 平均MFE=5.94, 平均MAE=6.16 (n=37)
...
- SWEEP_WAIT: 勝率=70.4%, 平均MFE=6.16, 平均MAE=6.62 (n=27) / データ不足 27/30
...
### bias別 direction 正誤
- long: correct=12, wrong=15, unclear=9 / wrong_rate=41.7% (n=36)
- short: correct=0, wrong=1, unclear=0 / wrong_rate=100.0% (n=1)

### 成績指標
- 全体勝率: 75.7%
- 平均MFE(signal_based): 5.94
- 平均MAE(signal_based): 6.16
...
- orderbook_ask_heavy: wrong_rate=40.0% (wrong=8/20)
- lower_liquidity_close: wrong_rate=39.4% (wrong=13/33)
...
- lower_liquidity_close: negative_rate=66.7% (n=33)
