...
### 2026-03-16 00:11 JST 時点

- ユーザー観測では、`Ver01` 基準で `2026-03-15 17:05 / 19:05 / 21:05 / 23:05 JST` に「相場の変化メール」が届いていた
- この 4 通は、ポジション方向の予測自体は外れていても、「相場が変わり始めた節目を拾えた」という意味で有効な比較基準とみなす
- したがって `Ver02.1` の注意報は、本命通知の代用品ではなく「変化検知センサー」として再調整する方針が妥当
- ローカル `Ver02.1 CLI` の `2026-03-15` ログを確認すると、`17:05` は `long / NO_TRADE_CANDIDATE / L59 S19 / gap40` で現在ロジックでも注意報対象だった
- `19:05` 近傍は `19:09` に `wait / RISKY_ENTRY / L59 S54 / gap5` で、`Ver02.1` では方向感が崩れて保留に見えており、注意報条件には入っていなかった
- `21:05` は `long / SWEEP_WAIT / L59 S41 / gap18` で、さらに少し前の `20:36` 時点で `long / SWEEP_WAIT / L59 S19 / gap40` により注意報条件へ入っていた
- `23:05` は `long / NO_TRADE_CANDIDATE / L59 S19 / gap40` で、変化継続は見えているが、現在ロジックは「初回の閾値突破」を重視するため追加通知にはつながりにくい
- つまり `Ver02.1` は相場変化を全く見逃しているわけではなく、「Ver01 のように節目ごとへ再通知しにくい」ことが差分の中心

### 注意報しきい値の再生検証

- ローカル `logs/signals/*.json` 142 件を使い、`should_notify()` を設定違いで再生した
- 現在の実効値は `.env` に個別設定が無いため、デフォルト `ATTENTION_ALERT_SCORE_MIN=55`、`ATTENTION_ALERT_GAP_MIN=15`、`ATTENTION_ALERT_COOLDOWN_MINUTES=60` が使われている
- 現在設定 `55 / 15 / 60` では注意報 `10件`
- `53 / 12 / 60` では注意報 `14件`
- `52 / 12 / 60` では注意報 `16件`
- `50 / 10 / 60` では注意報 `21件`
- `2026-03-15` 単日で見ると、現在設定でも `13:05 / 15:05 / 17:05 / 20:36` に注意報候補が出ていた
- `52 / 12` へ下げると、同日の `09:05` と `11:05` も拾えるため、「もう少し早く相場変化を見たい」という目的に近づく
- 一方 `50 / 10` はさらに早く拾えるが、通知数の増え方が大きく、ノイズ増加リスクが高い
...
```env
ATTENTION_ALERT_SCORE_MIN=52
ATTENTION_ALERT_GAP_MIN=12
ATTENTION_ALERT_COOLDOWN_MINUTES=30
```

- `52 / 12` は `Ver01` のような「変化の初動」を今より数時間早く見やすくしつつ、`50 / 10` ほど過敏ではない
- `ATTENTION_ALERT_COOLDOWN_MINUTES=30` は、相場変化が連続する局面で次の節目を見落としにくくするため
- ただし、しきい値調整だけでは `23:05` のような「同方向で変化継続中の強まり」を節目ごとに再通知しにくい問題は残る
- そのためコード側でも、同方向のまま `display_score` 上昇、`score_gap` 拡大、`prelabel` 変化などを再通知条件に追加する改善が必要
...
- 加えて、`Ver01` 比較で重要なのは「売買方向の正否」だけではなく「相場変化の始まりを何回拾えたか」である
- その観点では、`Ver02.1` は相場変化の兆し自体はかなり見えているが、注意報の節目通知が `Ver01` より控えめ
- したがって次の実務判断は、「注意報しきい値を `52 / 12 / 30` へ寄せる」ことと、「同方向の強まり再通知ロジックを足す」ことを優先する
...
- `ATTENTION_ALERT_SCORE_MIN=52`、`ATTENTION_ALERT_GAP_MIN=12`、`ATTENTION_ALERT_COOLDOWN_MINUTES=30` を適用した場合に、`Ver01` の変化メール時刻へどれだけ近づくか
- 同方向の `display_score` 上昇、`score_gap` 拡大、`prelabel` 変化を再通知条件に入れた場合、過剰通知にならず節目を拾えるか
