...
- `phase1_observation_gate=pass`: 8 件
- `direction_rr_learning`: 0 件
- `setup_watch_learning`: 8 件、近似PF 0.86、TP1先行 37.5%
- `confidence_watch_learning` の限定昇格候補: 1 件、近似PF 0.12、TP1先行 0.0%
...
- `confidence_below_min` でも、`watch + SWEEP_WAIT/RISKY_ENTRY + sweep_incomplete + lower_liquidity_close + 補助 hard flag なし + direction>=55 + execution>=18 + wait<=85` を満たす少数群だけは `confidence_watch_learning` 候補として別枠追跡する
- これは `Phase 1B` へ即昇格させる意味ではなく、「どの条件なら watch から実行候補へ近づけるか」を観測するための限定母集団
...
現時点では `direction_rr_learning` は新規帯で増えていないため、優先度を下げます。
`setup_watch_learning` は成績が弱いため、まず entry zone 未到達率と待機条件の改善対象として扱います。
`confidence_watch_learning` は `2026-04-29` 時点で 2 件しか抽出できず成績も弱いため、すぐに gate 緩和へ進まず「候補の再現有無」を先に見る段階です。
...
- `daily-sync` と `build-phase1b-promotion-report` は `confidence_watch_learning` の候補数と成績も表示する。
...
- `confidence_watch_learning` の件数、近似PF、TP1先行率
