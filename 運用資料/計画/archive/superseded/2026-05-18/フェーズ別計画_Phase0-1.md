...
- `Phase 1` は `Phase 0` の通知待ちと並行で導入を進めており、本有効確認は前倒しで進める
...
- `logic_validated` は自動計算実装済みで、レビューは改善補助として最低限の反映確認を行う
...
5. `通知評価シート.md` に `actual_move_driver` を入れ、`logic_validated` を反映
6. レビューは最低 5 件を確認し、それ以上は `Phase 1` 進行の停止条件にしない
7. 2週間以上 / 50件以上 / regime 多様性 / 週次レポート2回正常生成の確認
...
- `daily-sync` の `Phase 1 判定サマリー` で本有効件数と proxy 指標が読める確認
...
- レビュー件数は `Phase 1` 判断の主条件にしない。最低 5 件あれば補助観測として十分とする
