...
- 2026-03-13 10:30 JST 時点で、定期比較レポートの成功率集計は `Ver02.1` 以降だけへ仕切り直した。現時点では `Ver02.1 API` の新しい本番スナップショットがまだ無く、API 側母数は 0 件。`Ver02.1 CLI` はローカル全体 17 件中 6 件成功だが、`[CLI]` 明示ラベル後は 4 / 4、sandbox は 6 / 6 成功で、直近安定度は良好。
...
- 3. 本番は API、開発は CLI の役割分担のまま、次は API 本番の新しいスナップショットを pull して `Ver02.1 API` の母数を追加し、`運用資料/reports/cli_api定期比較レポート.md` の数値比較を更新する。
