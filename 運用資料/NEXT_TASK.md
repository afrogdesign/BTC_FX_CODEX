...
- `Ver02.5-v8` は紙ポジション状態遷移、TP2 / SL / timeout / missed_opportunity / entry_not_reached、日次レポートの closed 成績集計を追加する版。最新 commit `85914b4` を `origin/ver02.5-v8` へ push 済み。全体テスト 178 件 OK。
- 常駐 `com.afrog.btc-monitor` は `Ver02.5-v8` 反映後に再起動済み。PID `28498`、`state=running`、`monitor.err` は空。
...
- `Phase 1B` の実行候補はまだ 0 件。`feedback_daily_sync_20260519.md` でも `trade_execution_gate=pass=0件`、`paper_orders planned=0件`。
...
- メール件名ラベルは `Ver02.5-v6` で実送信確認済み。通常監視は `20260515_230500`、注意報は `20260516_060500` で新ランク表示を確認済み。`Ver02.5-v8` は再起動後の次回送信で確認する。
- `market_map` は shadow 側でも値入り確認済み。`market_map_effectiveness_20260518.md` では 116 件記録あり。
- AI 事後評価は `request_failed=0` を維持。`feedback_daily_sync_20260519.md` では backlog 77 件。
...
1. 次回サイクルで `SYSTEM_LABEL=Ver02.5-v8`、`execution_precision_*`、`15分足 執行チェック` が result / CSV / 詳細HTMLへ入るか確認する。
2. 次回 `daily-sync` で `paper_positions.csv` の `closed / TP / SL / timeout / missed_opportunity` 集計が継続して増えるか確認する。
...
- `com.afrog.btc-monitor` は `Ver02.5-v8` 反映後に再起動済み。PID `28498`、`state=running`、`monitor.err` は空。`logs/heartbeat.txt` と `logs/last_result.json` は次回定刻サイクルで更新確認する。
