...
- `daily-sync` の最新レポートは `運用資料/reports/feedback_daily_sync_20260416.md`。完了データは 39 件、近似PF は 1.04、全体勝率は 74.4%。
...
- `paper_orders.csv` への紙トレード planned 記録を追加済み。対象は `phase1_active=true` かつ `trade_execution_gate=pass` のみ。
...
2. 次の実装論点は、新規ログで `phase1_v1_shadow` が `tp_eval=too_close` の改善候補になっているか観測すること。
