...
- `daily-sync` の最新レポートは `運用資料/reports/feedback_daily_sync_20260422.md`。完了データは 27 件、近似PF は 0.84、全体勝率は 59.3%。
- AI 事後評価 health は `eligible=165`、`AI済み=123`、`backlog=42`、`created=4`、`request_failed=0`。レポート側でも直近 runtime の同期結果を拾うように修正済み。
- `phase1_observation_gate=pass` は 16 件。内訳は `direction_rr_learning=11件`、`setup_watch_learning=5件`。
...
2. `RISKY_ENTRY + rr_below_min かつ execution>=20` の補助集計を見て、`20260417_090500` 系が増えるか単発で終わるかを確認する。
...
- `trade_execution_gate=pass` はまだ 0 件で、Phase 1 の本有効条件は未達。観測 gate が出ていても、実行候補とは分けて判断する必要がある。
