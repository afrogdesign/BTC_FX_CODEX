...
- フェーズはまだ `Ver02.3 / Phase 0 本番観測中`。ただし次段階の主対象はレビュー蓄積ではなく `Phase 1` の本有効確認へ移した。
...
- 詳細 HTML の再検討ラインチャートはスマホ表示で縦方向が詰まらないように再調整した。価格レンジの縦配置をブロック全体へ広げ、LONG 吹き出しはロング帯の下へ逃がし、`無効` の絵文字は `⛔️` に統一した。
...
2. AI を通知時のみの監査役に切り替えた後、`main` / `attention` 通知でだけ `ai_audit` が保存され、非通知サイクルでは `skipped_non_notify` になることを確認する。
...
6. レビューは最低 5 件を維持し、それ以上は `Phase 1` 前進の停止条件にしない。`actual_move_driver` / 誤読確認が取れた行を優先する。
7. 実送信で確認できた HTML 詳細ページの見え方を実メール導線で見直し、再検討ラインチャートのスマホ視認性と `AI監査メモ` の出現条件が妥当かを確認する。
