...
- 単体テスト（`unittest`）は 8 件で、現時点は全件成功。
- 開発環境の常駐 `com.afrog.btc-monitor` は再起動済みで、最新コードが反映された状態。
- 打ち合わせノート参照先、ブランチ運用ルール、タグ運用ルール、Obsidian 側の `NEXT_TASK.md` リンク整備は完了済み。
- プロジェクト用 `AGENTS.md` は `btc_monitor/AGENTS.md` へ移動済み。

## 次のタスク
- 1. 近場サポレジ表示の補正後ログを 2〜3 サイクル追って、実メールと実チャートで「上下逆混在」が解消したか確認する。
...
- 4. `location_risk` / `risk_flags` は現状やや厳しめに効いているため、観察サンプルをもう少し増やしてから、止めすぎかどうかを判断する。
