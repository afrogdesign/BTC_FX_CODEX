...
- AI 事後評価は `request_failed=0` を維持。`feedback_daily_sync_20260601.md` では `created=8 / request_failed=0 / backlog=62`、`operational_focus_20260526.md` では未処理候補 38 件。
...
- `paper_positions.csv` は daily-sync 集計で `closed=14件`、`sl_hit=6件`、`missed_opportunity=6件`、`timeout=1件`、`tp2_hit=1件`、24h超 pending 0件。`market_map_opportunity` は 3件 / 勝率 0.0% / 平均R 1.30 / 簡易PF 0.00。
...
- AI 事後評価 health は `feedback_daily_sync_20260601.md` 基準で `eligible=393 / AI済み=331 / backlog=62 / created=8 / request_failed=0`。daily cap 8 は継続可能。
...
1. `phase1_active=true` は今回 0 件へ戻り、`trade_execution_gate=pass=0件` と `paper_orders planned=0件` は継続。実弾 gate 緩和ではなく、本有効候補の再出現と再現性を優先して見る。
...
7. AI backlog は 62 件で `request_failed=0`。daily cap 8 を維持し、backlog が自然減するか確認する。
...
- `feedback_daily_sync_YYYYMMDD.md` を次回生成し、AI事後評価の `eligible / AI済み / backlog / created / request_failed` を更新する。現状は `request_failed=0` だが backlog は 62 件残っている。
