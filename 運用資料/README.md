...
- タグ番号は世代番号に合わせる。例: Ver01 の安定版は `v1.0`、Ver02 の最初の安定版は `v2.0`、Ver02 の軽微調整版は `v2.1`。
- push のたびにタグは付けない。「本番へ上げる時」「安定版として残したい時」など、節目だけに付ける。
...
- 常駐運用の正本は MBP2020 本番環境です。
- 開発環境 MBA15 は、常駐ではなく一時検証と手動確認に使います。
- Ver01 は比較基準、Ver02 は改善検証対象として、本番機で並走させます。
- 詳細ルールは `今後の運用ルール.md` を参照してください。
...
- 本番常駐を変更した場合は、再起動後に `launchctl print ...` で `running / pid` を確認してください。
