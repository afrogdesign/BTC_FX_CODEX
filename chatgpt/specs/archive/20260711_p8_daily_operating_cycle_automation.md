# P8 Daily Operating Cycle Automation — Archived Specification

## Metadata

- work_id: `BTCFX-20260711-P8-DAILY-CYCLE-AUTOMATION`
- status: completed / runtime-applied / scheduled cycle verified
...
Run one accepted P8 evidence cycle each day without manual lineage reconstruction. The wrapper resolves the JST date, validates optional actual-evidence pairing, calls the accepted runner once, and writes a compact atomic last-result status. It does not reimplement P4/P5/P8 logic or apply tuning.

## CLI contract

`tools/run_p8_daily_cycle.py` accepts `--date YYYYMMDD` and `--dry-run` plus test-only path overrides. It invokes `tools/log_feedback.py run-p8-operating-cycle` with current candidates, `trades.csv`, explicit public OHLCV, date output root `logs/p8_operating_cycles/YYYYMMDD/`, `--replace-output`, and `--stdout-json`, using an argv list and no shell string.
...
## Scheduled verification result

The first scheduled cycle completed successfully on `2026-07-12` at 11:30 JST.
...
- counterfactual B: 9
- counterfactual C: 29
- actual evidence: missing / proxy-only
- initial P9 readiness: false
- practical P9 readiness: false
- error codes: none

The scheduled cycle verifies the runtime automation contract. It does not establish P9 readiness and does not authorize production tuning.

## Archive condition

Completed. Runtime apply and the first scheduled cycle were both verified. Generated evidence remains local and uncommitted.

## Safety boundary

No classifier, gate, threshold, scoring, notification, mail, API, account, private endpoint, order behavior, automatic tuning, P9 activation, raw export, or `paper_positions.csv` integration was changed.