...
class SummaryFormatTest(unittest.TestCase):
    def test_ready_case_separates_direction_and_execution(self) -> None:
        ...
        self.assertIn("【BTC:70,356】 2026-03-11 09:05", subject)
        ...
        self.assertIn("今の扱い: 条件付きで検討", body)
        ...
        self.assertIn("今の扱い", body)
        self.assertIn("確認条件", body)
        self.assertIn("TP / SL", body)
        self.assertIn("見送り・無効化条件", body)
        self.assertIn("有効期限", body)
        self.assertIn("安全境界", body)
    ...
    def test_attention_subject_and_body_are_wait_first(self) -> None:
        ...
        self.assertEqual(provider_used, "api")
        self.assertTrue(subject.startswith(f"{CURRENT_EMAIL_SUBJECT_PREFIX} 👀 [注意報・売買非推奨] 下方向バイアス | 重要な価格帯の近くで短期ノイズが出やすい "))
    ...
    def test_machine_only_subject_warns_and_body_hides_internal_codes(self) -> None:
        ...
        self.assertTrue(subject.startswith(f"{CURRENT_EMAIL_SUBJECT_PREFIX} 🟠 [高優先監視・実行不可] 下方向監視 | 近い買い板があり短期ノイズに注意 "))
    ...
    def test_build_summary_subject_retires_legacy_operator_version_labels(self) -> None:
        payload = {
        ...
        self.assertNotIn("Ver04-v1", subject)
        self.assertNotIn("Ver04-v2", subject)
        self.assertNotIn("[BTCFX Ver03-v4]", subject)
