...
class ManualOperatorClassifierTests(unittest.TestCase):
    ...
    def test_c_watch_candidate_status_and_absent_setup_side(self) -> None:
        result = self.run_classifier(self.fixture(status="candidate", direction="40", extra_signal={"primary_setup_side": "long"}))
        self.assertEqual(result["class_counts"].get("C_WATCH_ZONE"), 1)
        self.assertIn("c_watch_candidate_status_observation", self.read_rows()[0]["reason_codes"])
        for setup_side in ("", "none"):
            with self.subTest(setup_side=setup_side):
                result = self.run_classifier(self.fixture(status="candidate", direction="40", extra_signal={"primary_setup_side": setup_side}))
                self.assertEqual(result["class_counts"].get("C_WATCH_ZONE"), 1)
                self.assertIn("c_watch_primary_setup_side_absent", self.read_rows()[0]["reason_codes"])

    def test_c_watch_opposite_setup_side_and_unknown_status_fail_closed(self) -> None:
        result = self.run_classifier(self.fixture(status="candidate", direction="40", extra_signal={"primary_setup_side": "short"}))
        self.assertEqual(result["classification_status_counts"].get("insufficient_evidence"), 1)
        self.assertEqual(self.read_rows()[0]["reason_codes"], "side_mismatch")
