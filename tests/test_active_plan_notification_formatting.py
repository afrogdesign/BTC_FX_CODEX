...
class ActivePlanNotificationFormattingTest(unittest.TestCase):
    ...
    def _run_export_current_manual_delivery_app_surface_main_with_argv(
        self,
        argv: list[str],
        base_dir: Path | None = None,
    ) -> tuple[int, str, str]:
        ...
        with mock.patch.object(log_feedback, "BASE_DIR", resolved_base_dir):
            with mock.patch.object(
                sys,
                "argv",
                [str(BASE_DIR / "tools" / "log_feedback.py"), "export-current-manual-delivery-app-surface", *argv],
            ):
                with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                    try:
                        log_feedback.main()
    ...
    def _run_refresh_and_check_current_manual_delivery_app_surface_main_with_argv(
        self,
        argv: list[str],
        base_dir: Path | None = None,
    ) -> tuple[int, str, str]:
        ...
        with mock.patch.object(log_feedback, "BASE_DIR", resolved_base_dir):
            with mock.patch.object(
                sys,
                "argv",
                [str(BASE_DIR / "tools" / "log_feedback.py"), "refresh-and-check-current-manual-delivery-app-surface", *argv],
            ):
                with contextlib.redirect_stdout(stdout), contextlib.redirect_stderr(stderr):
                    try:
                        log_feedback.main()
    ...
    def test_write_current_manual_delivery_app_dashboard_cli_supports_help_and_writes_static_html(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("Safety Flags", dashboard_text)
            self.assertIn("Surface Roles", dashboard_text)
            self.assertIn("Ready Gate Copy", dashboard_text)
            self.assertIn("human_review_required", dashboard_text)
            self.assertIn("trade_execution_allowed", dashboard_text)
            self.assertIn("automatic_order_allowed", dashboard_text)
            self.assertIn("external_notification_allowed", dashboard_text)
            self.assertIn("paper_positions_integration", dashboard_text)
            self.assertIn("Public HTML notification report", dashboard_text)
            self.assertIn("Notification mail", dashboard_text)
            self.assertIn("Local dashboard / app surface", dashboard_text)
            self.assertIn("Single source of truth", dashboard_text)
            self.assertIn("same decision source, no separate decision path", dashboard_text)
            self.assertIn("manual_trade_ready=", dashboard_text)
            self.assertIn("readiness_status=", dashboard_text)
            self.assertIn("allowed_next_action=", dashboard_text)
            self.assertIn("dashboard=local/manual_delivery_app_surface/app-dashboard.html", dashboard_text)
            self.assertIn("snapshot=local/manual_delivery_app_surface/app-snapshot.json", dashboard_text)
            self.assertIn("manifest=local/manual_delivery_app_surface/app-surface-manifest.json", dashboard_text)
            self.assertIn("safety_boundary=report-only / not FORMAL_GO / no automatic order / human decides manually", dashboard_text)
    ...
    def test_refresh_current_manual_delivery_app_cli_supports_dashboard_output_and_stdout_json_mode(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("--app-dashboard-html", help_stdout)
            self.assertIn("--export-app-surface", help_stdout)
    ...
    def test_export_current_manual_delivery_app_surface_cli_supports_help_and_writes_bundle_and_refresh_export_mode(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                os.chdir(base_dir)
                help_code, help_stdout, help_stderr = self._run_export_current_manual_delivery_app_surface_main_with_argv(
                    ["--help"],
                    base_dir=base_dir,
                )
                ...
                refresh_export_code, refresh_export_stdout, refresh_export_stderr = self._run_refresh_current_manual_delivery_app_main_with_argv(
                    [
                        "--export-app-surface",
                        "--app-surface-dir",
                        str(refresh_export_dir),
                    ],
                    base_dir=base_dir,
                )
                stdout_json_export_code, stdout_json_export_stdout, stdout_json_export_stderr = self._run_refresh_current_manual_delivery_app_main_with_argv(
                    [
                        "--stdout-json",
                        "--export-app-surface",
                        "--app-surface-dir",
                        str(stdout_json_export_dir),
                    ],
                    base_dir=base_dir,
                )
            ...
            self.assertIn('href="app-dashboard.html"', index_text)
            self.assertIn('href="app-ready.json"', index_text)
            ...
            self.assertEqual(app_ready_data["schema_version"], "manual_delivery_app_ready_check.v1")
            self.assertTrue(app_ready_data["current_manual_delivery_app_ready"])
            self.assertEqual(app_ready_data["readiness_status"], "ready_for_human_review")
            self.assertEqual(app_ready_data["allowed_next_action"], "human_review_only")
            self.assertFalse(app_ready_data["trade_execution_allowed"])
            self.assertFalse(app_ready_data["automatic_order_allowed"])
            self.assertFalse(app_ready_data["external_notification_allowed"])
            self.assertFalse(app_ready_data["paper_positions_integration"])

            app_contract_data = json.loads((export_dir / "app-contract.json").read_text(encoding="utf-8"))
            self.assertEqual(app_contract_data["schema_version"], "manual_delivery_app_integration_contract.v1")
            self.assertEqual(app_contract_data["contract_status"], "stable_for_local_app_integration")
            self.assertEqual(app_contract_data["default_app_surface_dir"], "local/manual_delivery_app_surface")
            self.assertEqual(app_contract_data["surface_export_command"], "export-current-manual-delivery-app-surface")
            self.assertEqual(app_contract_data["refresh_surface_export_command"], "refresh-current-manual-delivery-app --export-app-surface")
            self.assertEqual(app_contract_data["surface_validation_command"], "check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(app_contract_data["surface_ready_gate_command"], "refresh-and-check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(app_contract_data["surface_validation_schema_version"], "manual_delivery_app_surface_validation.v1")
            self.assertEqual(app_contract_data["surface_ready_gate_stdout_mode"], "json_only")
            self.assertEqual(app_contract_data["surface_index_file"], "local/manual_delivery_app_surface/index.html")
            self.assertEqual(app_contract_data["surface_dashboard_file"], "local/manual_delivery_app_surface/app-dashboard.html")
            self.assertEqual(app_contract_data["surface_ready_file"], "local/manual_delivery_app_surface/app-ready.json")
            ...
            self.assertIn(str(refresh_export_dir), refresh_export_stdout)
            self.assertTrue((refresh_export_dir / "index.html").exists())
            self.assertTrue((refresh_export_dir / "app-dashboard.html").exists())
            self.assertTrue((refresh_export_dir / "app-ready.json").exists())
            ...
            self.assertNotIn("current_manual_delivery_app_refresh_dir=", stdout_json_export_stdout)
            self.assertNotIn("current_manual_delivery_app_surface_dir=", stdout_json_export_stdout)
            stdout_json_export_data = json.loads(stdout_json_export_stdout)
            self.assertEqual(stdout_json_export_data["schema_version"], "manual_delivery_app_ready_check.v1")
            self.assertTrue(stdout_json_export_data["current_manual_delivery_app_ready"])
            self.assertEqual(stdout_json_export_data["readiness_status"], "ready_for_human_review")
            self.assertEqual(stdout_json_export_data["allowed_next_action"], "human_review_only")
            self.assertFalse(stdout_json_export_data["trade_execution_allowed"])
            self.assertFalse(stdout_json_export_data["automatic_order_allowed"])
            self.assertFalse(stdout_json_export_data["external_notification_allowed"])
            self.assertFalse(stdout_json_export_data["paper_positions_integration"])
            self.assertTrue((stdout_json_export_dir / "index.html").exists())
            self.assertTrue((stdout_json_export_dir / "app-dashboard.html").exists())
            self.assertTrue((stdout_json_export_dir / "app-ready.json").exists())
    ...
    def test_export_current_manual_delivery_app_surface_ready_gate_contract_tracks_valid_absent_and_sanitized_post_eval_payloads(self) -> None:
        with TemporaryDirectory() as tmpdir:
            base_dir = Path(tmpdir)
            handoff_dir = _bootstrap_manual_delivery_handoff_fixtures(base_dir)
            export_dir = base_dir / "local" / "manual_delivery_app_surface"
            ...
            app_ready_data = json.loads((export_dir / "app-ready.json").read_text(encoding="utf-8"))
            app_contract_data = json.loads((export_dir / "app-contract.json").read_text(encoding="utf-8"))
            app_snapshot_data = json.loads((export_dir / "app-snapshot.json").read_text(encoding="utf-8"))
            check_data = json.loads(check_stdout)
            ...
            self.assertNotIn("automatic_order_allowed=true", combined_text)
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(export_code, 0, msg=export_stderr)
            self.assertEqual(check_code, 0, msg=check_stderr)
            absent_ready_data = json.loads((export_dir / "app-ready.json").read_text(encoding="utf-8"))
            absent_check_data = json.loads(check_stdout)
            ...
            self.assertEqual(absent_ready_data["post_eval_recommendations_reason_codes"], [])
    ...
    def test_export_current_manual_delivery_app_surface_marks_invalid_post_eval_payloads_not_ready(self) -> None:
        cases = [
            (
                "malformed",
                {
            ...
            (
                "wrong_schema",
                {
                    **_post_eval_recommendation_payload(),
            ...
            (
                ...
                {
                    **_post_eval_recommendation_payload(),
                    "safety_boundary": "manual review only",
                },
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                os.chdir(base_dir)
                for case_name, payload, expected_reason_code in cases:
                    export_dir = base_dir / "local" / f"{case_name}-manual_delivery_app_surface"
                    payload_path = _write_json_fixture(base_dir / "fixtures" / f"{case_name}-post-eval.json", payload)
                    with mock.patch.object(
                        log_feedback,
                        "build_post_eval_recommendation_report",
                        side_effect=AssertionError("recommendation engine must not run during dashboard export"),
                    ...
                    self.assertEqual(export_code, 0, msg=export_stderr)
                    self.assertEqual(check_code, 0, msg=check_stderr)
                    ready_data = json.loads((export_dir / "app-ready.json").read_text(encoding="utf-8"))
                    check_data = json.loads(check_stdout)
                    self.assertFalse(ready_data["post_eval_recommendations_ready"], msg=case_name)
                    self.assertEqual(ready_data["post_eval_recommendations_status"], "invalid_not_ready")
                    self.assertIn(expected_reason_code, ready_data["post_eval_recommendations_reason_codes"])
                    self.assertFalse(check_data["post_eval_recommendations_ready"], msg=case_name)
                    self.assertEqual(check_data["post_eval_recommendations_status"], "invalid_not_ready")
                    self.assertIn(expected_reason_code, check_data["post_eval_recommendations_reason_codes"])
            finally:
                os.chdir(original_cwd)

    def test_refresh_and_check_current_manual_delivery_app_surface_cli_supports_help_default_and_stdout_json_modes(self) -> None:
        with TemporaryDirectory() as tmpdir:
            base_dir = Path(tmpdir)
            surface_dir = base_dir / "local" / "manual_delivery_app_surface"
            original_cwd = Path.cwd()
            try:
                os.chdir(base_dir)
                help_code, help_stdout, help_stderr = self._run_refresh_and_check_current_manual_delivery_app_surface_main_with_argv(
                    ["--help"],
                    base_dir=base_dir,
                )
                default_code, default_stdout, default_stderr = self._run_refresh_and_check_current_manual_delivery_app_surface_main_with_argv(
                    [],
                    base_dir=base_dir,
                )
                stdout_json_code, stdout_json_stdout, stdout_json_stderr = self._run_refresh_and_check_current_manual_delivery_app_surface_main_with_argv(
                    [
                        "--stdout-json",
                    ],
                    base_dir=base_dir,
            ...
            self.assertEqual(help_code, 0, msg=help_stderr)
            self.assertIn("refresh-and-check-current-manual-delivery-app-surface", help_stdout)
            self.assertIn("--app-surface-dir", help_stdout)
            ...
            self.assertEqual(
                default_stdout,
                "current_manual_delivery_app_surface_ready=true\n"
                "current_manual_delivery_app_surface_dir=local/manual_delivery_app_surface\n"
                "current_manual_delivery_app_surface_status=valid_ready_for_human_review\n",
            )
            self.assertTrue(surface_dir.exists())
            self.assertTrue((surface_dir / "index.html").exists())
            self.assertTrue((surface_dir / "app-dashboard.html").exists())
            self.assertTrue((surface_dir / "app-ready.json").exists())
            ...
            self.assertEqual(stdout_json_code, 0, msg=stdout_json_stderr)
            self.assertTrue(stdout_json_stdout.startswith("{\n"))
            self.assertNotIn("current_manual_delivery_app_surface_ready=true", stdout_json_stdout)
            self.assertNotIn("current_manual_delivery_app_surface_dir=", stdout_json_stdout)
            self.assertNotIn("current_manual_delivery_app_surface_status=", stdout_json_stdout)
            stdout_json_data = json.loads(stdout_json_stdout)
            self.assertEqual(stdout_json_data["schema_version"], "manual_delivery_app_surface_validation.v1")
            self.assertEqual(stdout_json_data["surface_status"], "valid_ready_for_human_review")
            self.assertEqual(stdout_json_data["app_surface_dir"], "local/manual_delivery_app_surface")
            self.assertEqual(stdout_json_data["index_html"], "local/manual_delivery_app_surface/index.html")
            self.assertEqual(stdout_json_data["app_dashboard_html"], "local/manual_delivery_app_surface/app-dashboard.html")
            self.assertEqual(stdout_json_data["app_ready_json"], "local/manual_delivery_app_surface/app-ready.json")
    ...
    def test_refresh_current_manual_delivery_app_surface_launcher_script_exists_and_is_shell_safe(self) -> None:
        ...
        self.assertIn("local/manual_delivery_app_surface/app-dashboard.html", script_text)
        self.assertIn("manual_trade_ready", script_text)
        self.assertIn("readiness_status", script_text)
        self.assertIn("allowed_next_action", script_text)
        self.assertIn("human_review_required", script_text)
        self.assertIn("trade_execution_allowed", script_text)
        self.assertIn("automatic_order_allowed", script_text)
        self.assertIn("external_notification_allowed", script_text)
        self.assertIn("paper_positions_integration", script_text)
        self.assertIn("app_dashboard_html", script_text)
        self.assertIn("app_snapshot_json", script_text)
        self.assertIn("app_snapshot_status_json", script_text)
        self.assertIn("app_surface_manifest_json", script_text)
        self.assertIn("safety_boundary", script_text)
        self.assertTrue("json.loads" in script_text or "json.load" in script_text)
    ...
    def test_describe_current_manual_delivery_app_contract_cli_supports_help_and_output_modes(self) -> None:
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                stdout_json_only_code, stdout_json_only_stdout, stdout_json_only_stderr = self._run_describe_current_manual_delivery_app_contract_main_with_argv(
                    [
                        "--stdout-json",
                    ],
                    base_dir=base_dir,
            ...
            self.assertEqual(output_json_only_data["default_app_surface_dir"], "local/manual_delivery_app_surface")
            self.assertEqual(output_json_only_data["surface_export_command"], "export-current-manual-delivery-app-surface")
            self.assertEqual(output_json_only_data["refresh_surface_export_command"], "refresh-current-manual-delivery-app --export-app-surface")
            self.assertEqual(output_json_only_data["surface_validation_command"], "check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(output_json_only_data["surface_ready_gate_command"], "refresh-and-check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(output_json_only_data["surface_validation_schema_version"], "manual_delivery_app_surface_validation.v1")
            self.assertEqual(output_json_only_data["surface_ready_gate_stdout_mode"], "json_only")
            self.assertEqual(output_json_only_data["surface_index_file"], "local/manual_delivery_app_surface/index.html")
            self.assertEqual(output_json_only_data["surface_dashboard_file"], "local/manual_delivery_app_surface/app-dashboard.html")
            self.assertEqual(output_json_only_data["surface_ready_file"], "local/manual_delivery_app_surface/app-ready.json")
            ...
            self.assertEqual(stdout_json_json_only_data["default_app_surface_dir"], "local/manual_delivery_app_surface")
            self.assertEqual(stdout_json_json_only_data["surface_export_command"], "export-current-manual-delivery-app-surface")
            self.assertEqual(stdout_json_json_only_data["refresh_surface_export_command"], "refresh-current-manual-delivery-app --export-app-surface")
            self.assertEqual(stdout_json_json_only_data["surface_validation_command"], "check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(stdout_json_json_only_data["surface_ready_gate_command"], "refresh-and-check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(stdout_json_json_only_data["surface_validation_schema_version"], "manual_delivery_app_surface_validation.v1")
            self.assertEqual(stdout_json_json_only_data["surface_ready_gate_stdout_mode"], "json_only")
            ...
            self.assertTrue(both_md.exists())
            self.assertTrue(both_json.exists())
            self.assertEqual(stdout_json_only_code, 0, msg=stdout_json_only_stderr)
            self.assertTrue(stdout_json_only_stdout.startswith("{\n"))
            self.assertNotIn("current_manual_delivery_app_contract_md=", stdout_json_only_stdout)
            self.assertNotIn("# Manual Delivery Current App Integration Contract", stdout_json_only_stdout)
            stdout_json_only_data = json.loads(stdout_json_only_stdout)
            self.assertEqual(stdout_json_only_data, output_json_only_data)
            self.assertEqual(stdout_json_only_data["default_app_surface_dir"], "local/manual_delivery_app_surface")
            self.assertEqual(stdout_json_only_data["surface_export_command"], "export-current-manual-delivery-app-surface")
            self.assertEqual(stdout_json_only_data["refresh_surface_export_command"], "refresh-current-manual-delivery-app --export-app-surface")
            self.assertEqual(stdout_json_only_data["surface_validation_command"], "check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(stdout_json_only_data["surface_ready_gate_command"], "refresh-and-check-current-manual-delivery-app-surface --stdout-json")
            self.assertEqual(stdout_json_only_data["surface_validation_schema_version"], "manual_delivery_app_surface_validation.v1")
            self.assertEqual(stdout_json_only_data["surface_ready_gate_stdout_mode"], "json_only")
