...
- 最新の作業ブランチ名は常に `運用資料/NEXT_TASK.md` を正本とする。ブランチを切った作業では同じ作業単位で `NEXT_TASK.md` を更新する。
...
`chatgpt/specs/active/` が空のときは、ChatGPT は `NEXT_TASK.md` に書かれた継続メモを起点に `chatgpt/analysis/` を更新し、確定したものだけを `chatgpt/specs/active/` へ上げる。
...
7. `active/` が空なら `運用資料/ChatGPTプロジェクト設定.md` → `運用資料/NEXT_TASK.md` → `chatgpt/analysis/` を読む。
...
- Codex はこの案件で `AGENTS.md` → `chatgpt/README.md` → `chatgpt/initial_settings.md` → `chatgpt/specs/active/` → `運用資料/ChatGPTプロジェクト設定.md` → `運用資料/NEXT_TASK.md` の順で読む

## 案件の前提

- 現行主流の作業ブランチは `運用資料/NEXT_TASK.md` を見る
- `com.afrog.btc-monitor` の実行コード実体も、原則として `運用資料/NEXT_TASK.md` の現在の作業ブランチを正本とする。件名ラベルや節目版の表記と混同しない。
...
- ChatGPT の入口は `運用資料/ChatGPTプロジェクト設定.md` → `運用資料/NEXT_TASK.md` → `運用資料/reports/report_hub_latest.md` → raw report → `chatgpt/README.md` → `chatgpt/analysis/`
- Codex の入口は `AGENTS.md` → `chatgpt/README.md` → `chatgpt/specs/active/` → `運用資料/ChatGPTプロジェクト設定.md` → `運用資料/NEXT_TASK.md`
...
- ユーザーが `実行系まで反映して` と書いたときは、`運用資料/運用/実務/実行系反映_再起動_報告手順.md` を適用する
  - 実装完了だけで終えず、必要なレポート生成、`com.afrog.btc-monitor` 再起動、`launchctl` / PID / error 確認、`NEXT_TASK.md` / `progress.md` 更新、必要時 Global_BOX 更新、人向け報告、commit / push までを一連で閉じる
