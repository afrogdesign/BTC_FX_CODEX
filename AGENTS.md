...
設計が未確定な話は `chatgpt/analysis/` を確認し、勝手に実装へ進めない。ChatGPT の次判断や最新指示は常に `運用資料/NEXT_TASK.md` を正本とする。
...
- AI の入口は `AGENTS.md` → `chatgpt/README.md` → `chatgpt/specs/active/` → `運用資料/ChatGPTプロジェクト設定.md` → `運用資料/NEXT_TASK.md` → `運用資料/開発ロードマップ.md`
