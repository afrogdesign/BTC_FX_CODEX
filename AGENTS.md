...
- `次にやる` は 1〜3 件までに絞る。
- `今の状況` と `次にやる` は、新しく発生したこと、新しく完了したこと、新しく決まったことを常に先頭行へ置く。
- 前回から変わっていない状況や継続中の前提は、その下に回して並べる。
- `今の状況` は、箇条書きの事実羅列ではなく、人が次の人へ短く報告する言い方で書く。
- `今の状況` に古い情報を残しすぎない。新しい判断や新しい完了事項が出たら、不要になった古い行は消すか要約して圧縮する。
- `今の状況` は、読んだ瞬間に「いま何待ちか」「何が動いているか」が分かる軽さを優先する。
- `次に見る` は今すぐ開くべき資料リンクを 2〜5 件に絞る。
- `重要メモ` は今の判断に必要な前提だけを 1〜3 件書く。
- ブランチを新規作成した場合や、ブランチの役割を変えた場合は、その理由を `重要メモ` に短く残す。
...
- `NEXT_TASK.md` には「何をしたか」の時系列ログを書かない。実行履歴と完了報告は `progress.md` に書く。
- 同じ意味の項目を重複して残さない。1つの判断につき1行に圧縮する。
- 迷ったら削る。詳細説明や背景は `progress.md`、`開発ロードマップ.md`、`運用資料/reports/` へ逃がす。
- 次回やることを優先順で 3〜7 件書く。
