...
- current_work_id: `BTCFX-20260702-VER04-V1-RUNTIME-PREFLIGHT`
- mode: `NORMAL_CODEX`

## Current goal

Ver04-v1 の runtime preflight を read-only で実施し、runtime target の branch / upstream / worktree 状態を確認して、reflection に進めるかを判定する。

この task では runtime access は read-only の確認に限り、pull / sync / restart / launchd / notification sending / order execution は行わない。
...
Stop with `BLOCKED BTCFX-20260702-VER04-V1-RUNTIME-PREFLIGHT: <one specific question>` if:
...
- `BTCFX-20260702-VER04-V1-RUNTIME-PREFLIGHT-FIX`
- Runtime reflection is blocked until the runtime target is on Ver04-v1 with a configured upstream and a clean or explainable worktree.
