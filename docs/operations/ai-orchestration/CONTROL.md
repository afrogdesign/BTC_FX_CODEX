...
Current reviewed baseline is the current manual-delivery app stdout-json E2E confirmation after the current manual-delivery app refresh path E2E confirmation.
...
Canonical app/operator path: `refresh-current-manual-delivery-app` -> `check-current-manual-delivery-app-ready --stdout-json`; stable read file `local/manual_delivery_handoff/app-snapshot.json`; validator/status file `local/manual_delivery_handoff/app-snapshot-status.json`.
