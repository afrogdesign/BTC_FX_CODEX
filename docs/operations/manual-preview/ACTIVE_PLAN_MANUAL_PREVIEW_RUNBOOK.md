...
- Use `check-current-manual-delivery-app-ready --stdout-json` as the recommended app integration read mode after `refresh-current-manual-delivery-app`; it reads `app-snapshot-status.json` and prints the ready gate as deterministic JSON without compact lines.
...
- Use `summarize-current-manual-delivery-app-snapshot` as the app/operator read-side validator for `app-snapshot.json`.
- It does not send, notify, fetch, rebuild, trade, or approve anything.
