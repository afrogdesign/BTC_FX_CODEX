...
- `scripts/refresh_current_manual_delivery_app_surface.command` refreshes, exports, and validates the local app surface, then prints the static files to open manually. Run it from Terminal or Finder after checkout; it is tracked as executable. It does not open a browser or start a server.
- It does not send, notify, fetch, rebuild, trade, or approve anything.
