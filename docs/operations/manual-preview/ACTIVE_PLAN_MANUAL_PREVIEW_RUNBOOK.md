...
- 公開HTML notification report remains the current manual-trading main UI.
...
- Ver03-v4 notification mail subject uses `[BTCFX Ver03-v4]`, and the mail body includes the local app surface ready-gate and dashboard paths for manual-trade confirmation.
...
- `scripts/refresh_current_manual_delivery_app_surface.command` refreshes, exports, and validates the local app surface, then prints the static files to open manually plus a compact terminal operator summary after successful validation. Run it from Terminal or Finder after checkout; it is tracked as executable. It does not open a browser or start a server.
- It does not send, notify, fetch, rebuild, trade, or approve anything.
