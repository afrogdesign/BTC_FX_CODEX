...
## 1. まず結論
- 今回の完了データは 27 件です。近似PF は 1.35、全体勝率は 74.1% でした。
- 事後評価では「待つ判断に使えた」が最も多く、13 件でした。
- 平均の役立ち度は 3.57 / 5 でした。
- 根拠整合の入力率は 48.1%、整合した比率は 100.0% でした。
...
## 2. 今回の対象
- 集計期間: 2026-04-12 05:05 〜 2026-04-17 18:05
- 総観測件数: 27
- データ品質の内訳: ok=27
- 近似PF: 1.35
...
- ready阻害理由: rr_below_min=20件, entry_zone_not_reached=3件, inside_entry_zone_with_trigger=2件, confidence_below_min=1件, near_entry_zone_with_trigger=1件
...
- 根拠整合の入力率: 48.1% / 整合率: 100.0%
- SL評価: SL は妥当=8件, SL が広すぎた=3件, SL が狭すぎた=3件
- TP評価: TP が近すぎた=9件, TP が遠すぎた=2件, TP は妥当=3件
- 4時間足評価: 妥当=5件, 一部弱い=7件, 弱い=2件
- 1時間足評価: 一部弱い=9件, 妥当=5件
- 15分足評価: 弱い=3件, 妥当=4件, 一部弱い=7件
### 改善アクション
- 分類: 通知文面を調整=3件, 出口設計を調整=10件, リスク設計を調整=1件
- 重要度: 高=11件, 中=3件
- 高優先の代表例:
...
- uptrend: 勝率=57.1%, 平均MFE=2.35, 平均MAE=9.03 (n=7) / データ不足 7/30

### signal_tier別件数・勝率・平均MFE・平均MAE
- normal: 勝率=74.1%, 平均MFE=6.99, 平均MAE=5.17 (n=27) / データ不足 27/30
...
- SWEEP_WAIT: 勝率=62.5%, 平均MFE=6.82, 平均MAE=6.10 (n=16) / データ不足 16/30
- NO_TRADE_CANDIDATE: 勝率=100.0%, 平均MFE=3.38, 平均MAE=4.00 (n=1) / データ不足 1/30

### bias別件数・勝率
- long: 勝率=76.9% (n=26) / データ不足 26/30
- short: 勝率=0.0% (n=1) / データ不足 1/30

### bias別 direction 正誤
- long: correct=10, wrong=11, unclear=5 / wrong_rate=42.3% (n=26)
- short: correct=0, wrong=1, unclear=0 / wrong_rate=100.0% (n=1)

### 成績指標
- 全体勝率: 74.1%
- 平均MFE(signal_based): 6.99
- 平均MAE(signal_based): 5.17
...
- orderbook_ask_heavy: wrong_rate=50.0% (wrong=7/14)
- sweep_incomplete: wrong_rate=43.5% (wrong=10/23)
- lower_liquidity_close: wrong_rate=40.9% (wrong=9/22)
...
- tp_eval=too_close のうち shadow TP1 が現行TP1より遠い候補: 0/9件

### risk_flags 有効性比較
- sweep_incomplete: negative_rate=65.2% (n=23)
- lower_liquidity_close: negative_rate=63.6% (n=22)
- ask_wall_close: negative_rate=60.0% (n=10)
- orderbook_ask_heavy: negative_rate=57.1% (n=14)
