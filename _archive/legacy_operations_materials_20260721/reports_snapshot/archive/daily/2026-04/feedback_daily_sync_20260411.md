...
## 1. まず結論
- 今回の完了データは 20 件です。近似PF は 1.95、全体勝率は 85.0% でした。
- 事後評価では「待つ判断に使えた」が最も多く、8 件でした。
- 平均の役立ち度は 3.72 / 5 でした。
- 根拠整合の入力率は 45.0%、整合した比率は 100.0% でした。
...
## 2. 今回の対象
- 集計期間: 2026-04-04 20:05 〜 2026-04-10 01:05
- 総観測件数: 20
- データ品質の内訳: ok=20
- 近似PF: 1.95
...
- 根拠整合の入力率: 45.0% / 整合率: 100.0%

## 5. 改善候補
1. ENTRY_OK と setup invalid の整合性崩れ
   理由: 期間内で ENTRY_OK + invalid が 15 件あります
...
- transition: 勝率=87.5%, 平均MFE=10.06, 平均MAE=2.99 (n=8) / データ不足 8/30
- uptrend: 勝率=80.0%, 平均MFE=9.03, 平均MAE=2.72 (n=10) / データ不足 10/30

### signal_tier別件数・勝率・平均MFE・平均MAE
- normal: 勝率=85.0%, 平均MFE=9.08, 平均MAE=4.65 (n=20) / データ不足 20/30

### prelabel別件数・勝率・平均MFE・平均MAE
- ENTRY_OK: 勝率=60.0%, 平均MFE=7.36, 平均MAE=4.84 (n=5) / データ不足 5/30
- RISKY_ENTRY: 勝率=100.0%, 平均MFE=4.98, 平均MAE=2.19 (n=1) / データ不足 1/30
- SWEEP_WAIT: 勝率=92.3%, 平均MFE=9.34, 平均MAE=5.10 (n=13) / データ不足 13/30
- NO_TRADE_CANDIDATE: 勝率=100.0%, 平均MFE=18.44, 平均MAE=0.35 (n=1) / データ不足 1/30

### bias別件数・勝率
- long: 勝率=88.2% (n=17) / データ不足 17/30
- short: 勝率=66.7% (n=3) / データ不足 3/30

### bias別 direction 正誤
- long: correct=7, wrong=3, unclear=7 / wrong_rate=17.6% (n=17)
- short: correct=2, wrong=1, unclear=0 / wrong_rate=33.3% (n=3)

### 成績指標
- 全体勝率: 85.0%
- 平均MFE(signal_based): 9.08
- 平均MAE(signal_based): 4.65
...
- short_cover_risk: wrong_rate=20.0% (wrong=1/5)
- orderbook_ask_heavy: wrong_rate=16.7% (wrong=1/6)
- sweep_incomplete: wrong_rate=15.4% (wrong=2/13)
- lower_liquidity_close: wrong_rate=12.5% (wrong=2/16)
- ask_wall_close: wrong_rate=0.0% (wrong=0/9)
...
### risk_flags 有効性比較
- lower_liquidity_close: negative_rate=56.2% (n=16)
- sweep_incomplete: negative_rate=53.8% (n=13)
